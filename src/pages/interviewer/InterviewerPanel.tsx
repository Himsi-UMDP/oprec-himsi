...
import { LogOut, ClipboardList, X, Save, ChevronDown, BookOpen, CheckCircle2, ChevronRight } from 'lucide-react'
...
const INTERVIEW_QUESTIONS = [
  {
    kategori: 'Pembukaan & Umum', warna: 'bg-blue-50 text-blue-700 border-blue-200',
    pertanyaan: [
  ...
  {
    kategori: 'Wawasan Organisasi', warna: 'bg-purple-50 text-purple-700 border-purple-200',
    pertanyaan: [
  ...
  {
    kategori: 'Manajemen Waktu', warna: 'bg-amber-50 text-amber-700 border-amber-200',
    pertanyaan: [
  ...
  {
    kategori: 'Teamwork', warna: 'bg-green-50 text-green-700 border-green-200',
    pertanyaan: [
  ...
  {
    kategori: 'Leadership', warna: 'bg-orange-50 text-orange-700 border-orange-200',
    pertanyaan: [
  ...
  {
    kategori: 'Komitmen', warna: 'bg-red-50 text-red-700 border-red-200',
    pertanyaan: [
  ...
  {
    kategori: 'Pertanyaan Bidang', warna: 'bg-teal-50 text-teal-700 border-teal-200',
    pertanyaan: [
...
const nilaiToScores = (n: NilaiWawancaraRow): Scores => ({
  sikap: n.sikap ?? '', public_speaking: n.public_speaking ?? '',
  wawasan_org: n.wawasan_org ?? '', manajemen: n.manajemen ?? '',
  teamwork: n.teamwork ?? '', leadership: n.leadership ?? '',
  komitmen: n.komitmen ?? '', bidang: n.bidang ?? '',
})
...
const SCORE_COLORS = ['', 'bg-red-100 text-red-700 border-red-300', 'bg-orange-100 text-orange-700 border-orange-300', 'bg-yellow-100 text-yellow-700 border-yellow-300', 'bg-blue-100 text-blue-700 border-blue-300', 'bg-green-100 text-green-700 border-green-300']

function PanduanModal({ onClose }: { onClose: () => void }) {
  const [activeKat, setActiveKat] = useState(0)
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-hidden" onClick={onClose}>
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-2xl flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex flex-1 overflow-hidden min-h-0 overflow-hidden">
          <div className="w-44 border-r border-gray-100 flex flex-col overflow-y-auto shrink-0">
            {INTERVIEW_QUESTIONS.map((kat, i) => (
              <button key={i} onClick={() => setActiveKat(i)}
                className={['text-left px-3 py-3 text-xs font-bold border-b border-gray-50 transition-colors leading-tight', activeKat === i ? 'bg-blue-50 text-[#2464a8]' : 'text-gray-500 hover:bg-gray-50'].join(' ')}>
                {kat.kategori}
              </button>
            ))}
          </div>
          <div className="flex-1 overflow-y-auto p-5 space-y-3 overflow-hidden">
            ...
            {INTERVIEW_QUESTIONS[activeKat].pertanyaan.map((q, i) => (
              <div key={i} className="flex gap-3 p-3 rounded-xl bg-gray-50 hover:bg-blue-50/40 transition-colors overflow-hidden">
                <span className="w-6 h-6 rounded-lg bg-gray-200 text-gray-600 text-xs font-extrabold flex items-center justify-center shrink-0">{i + 1}</span>
...
function PenilaianModal({ row, scores, catatan, onClose, onSave, saving, penilaiName }: {
  row: PendaftaranRow; scores: Scores; catatan: string
  onClose: () => void; onSave: (s: Scores, c: string) => void; saving: boolean
  penilaiName: string
}) {
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm" onClick={onClose}>
        <div className="w-full sm:max-w-lg bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[92vh]" onClick={e => e.stopPropagation()}>
          <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100 shrink-0">
            <div className="w-9 h-9 rounded-xl bg-blue-50 text-[#2464a8] flex items-center justify-center shrink-0">
            ...
            <button onClick={() => setShowPanduan(true)} className="flex items-center gap-1.5 text-xs font-bold text-[#2464a8] bg-blue-50 hover:bg-blue-100 px-2.5 py-1.5 rounded-lg transition-colors mr-1 shrink-0">
          ...
          <div className="overflow-y-auto flex-1 px-5 py-5 space-y-3">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-wide">Klik angka untuk memberi nilai · 1 = Sangat Buruk, 5 = Sangat Baik</p>
              ...
              return (
                <div key={cat.key} className="rounded-xl border border-gray-100 bg-gray-50/50 p-3">
                  <div className="flex items-start justify-between gap-3 mb-2">
                    ...
                    {v !== '' && <span className={`text-xs font-bold px-2 py-0.5 rounded-full border shrink-0 ${SCORE_COLORS[v as number]}`}>{SCORE_LABELS[v as number]}</span>}
                  </div>
                  <div className="flex gap-2">
                    {[1, 2, 3, 4, 5].map(n => (
                      <button key={n} onClick={() => setLocal(p => ({ ...p, [cat.key]: p[cat.key] === n ? '' : n }))}
                        className={['flex-1 h-9 rounded-xl text-sm font-extrabold border-2 transition-all', local[cat.key] === n ? 'bg-[#2464a8] text-white border-[#2464a8] shadow-md scale-105' : 'bg-white text-gray-400 border-gray-200 hover:border-[#2464a8]/50'].join(' ')}>
                        {n}
                      </button>
                    ))}
                  </div>
            ...
            <div className={['rounded-xl border-2 px-4 py-3 flex items-center justify-between', avgNum !== null ? avgNum >= 4 ? 'border-green-200 bg-green-50' : avgNum >= 3 ? 'border-blue-200 bg-blue-50' : 'border-orange-200 bg-orange-50' : 'border-gray-100 bg-gray-50'].join(' ')}>
              <div>
                <p className="text-xs font-bold text-gray-500 uppercase tracking-wide">Rata-rata Nilai</p>
                <p className="text-xs text-gray-400 font-semibold">{Object.values(local).filter(v => v !== '').length} dari {SCORE_CATEGORIES.length} kategori dinilai</p>
              </div>
              <span className={['text-3xl font-extrabold', avgNum !== null ? avgNum >= 4 ? 'text-green-600' : avgNum >= 3 ? 'text-blue-600' : 'text-orange-600' : 'text-gray-300'].join(' ')}>{avg}</span>
            </div>
            <div>
              <label className="text-xs font-bold text-gray-500 uppercase tracking-wide block mb-2">Catatan Tambahan (opsional)</label>
              <textarea value={localCatatan} onChange={e => setLocalCatatan(e.target.value)}
          ...
          <div className="px-5 py-4 border-t border-gray-100 shrink-0">
            <button onClick={() => onSave(local, localCatatan)} disabled={saving}
              ...
              {saving ? <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Menyimpan...</> : <><Save className="w-4 h-4" />Simpan Penilaian</>}
            </button>
          </div>
        </div>
      </div>
...
export default function InterviewerPanel() {
  ...
  async function fetchAll() {
    setLoadingData(true)
    try {
      // 1) list penilai untuk akun ini (RLS filter by auth.email())
  ...
  const filtered = useMemo(() => pendaftar.filter(d => {
  ...
  async function handleSave(row: PendaftaranRow, scores: Scores, catatan: string) {
    ...
    try {
      const saved = await upsertNilai({
        ...
        interviewer_id: profile.id,       // UID akun
        interviewer_name: penilaiName,    // NAMA ORANG (bukan profile.nama)
  ...
  async function handleSetSelesai(row: PendaftaranRow) {
    ...
    try {
      // recommended: pakai RPC kalau kamu sudah buat
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200 px-5 py-3 flex items-center justify-between shadow-sm sticky top-0 z-10">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setShowPanduan(true)} className="flex items-center gap-1.5 text-xs font-bold text-[#2464a8] bg-blue-50 hover:bg-blue-100 px-3 py-2 rounded-xl transition-colors">
            <BookOpen className="w-3.5 h-3.5" /> Panduan
          </button>
          <button onClick={logout} className="flex items-center gap-1.5 text-xs font-bold text-white bg-[#2464a8] hover:bg-red-500 px-3 py-2 rounded-xl transition-colors">
      ...
      <main className="p-5 md:p-8 space-y-5 max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-3">
          {[
            ...
            { label: 'Selesai Wawancara', value: pendaftar.filter(p => p.status === 'selesai_wawancara').length, emoji: '🏁', bg: 'bg-indigo-50' },
          ].map(s => (
        ...
        <div className="rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm font-semibold text-blue-700">
          ℹ️ Klik <strong>"Nilai"</strong> untuk membuka form penilaian. Klik <strong>"Selesai"</strong> setelah wawancara peserta selesai.
        </div>

        <div className="flex flex-wrap gap-3 items-center">
          <input type="text" placeholder="Cari nama / NPM..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                  ...
                  {loadingData ? (
                    ...
                    return (
                      <tr key={row.id} className="border-b border-gray-50 hover:bg-blue-50/20 transition-colors">
                        ...
                        <td className="px-3 py-3"><span className="text-xs font-semibold text-gray-800 max-w-[130px] truncate block">{row.nama}</span></td>
                        <td className="px-3 py-3 text-gray-600 font-mono text-xs">{row.npm}</td>
                        <td className="px-3 py-3"><span className="rounded-full bg-blue-50 text-blue-700 text-xs font-bold px-2 py-0.5">{row.bidang1}</span></td>
                        <td className="px-3 py-3"><span className="rounded-full bg-purple-50 text-purple-700 text-xs font-bold px-2 py-0.5">{row.bidang2}</span></td>
                        ...
                        <td className="px-3 py-3">
                          <div className="flex items-center gap-1 flex-wrap">
                            <button onClick={() => setModalRow(row)}
                              ...
                              <ClipboardList className="w-3 h-3" />{sudahDinilai ? 'Edit' : 'Nilai'}
                            </button>
                            {!sudahSelesai && (
                              <button onClick={() => handleSetSelesai(row)} disabled={updatingStatusId === row.id}
                                ...
                                <ChevronRight className="w-3 h-3" />{updatingStatusId === row.id ? '...' : 'Selesai'}
                              </button>
                            )}
                          </div>
                        </td>
