...
import { Download, X, User, FileText, Trash2 } from "lucide-react";
...
type Props = {
  ...
  onDelete: (row: PendaftaranRow) => void;
};
    ...
    >
      ...
      >
        <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100 shrink-0">
          ...
          <p className="font-extrabold text-gray-800 text-sm flex-1 leading-tight">
            {title}
          </p>
          <button
            onClick={onClose}
...
function ClickableCell({ text, onOpen }: { text: string; onOpen: () => void }) {
  return (
    <button
      type="button"
      onClick={onOpen}
      className="text-left group block max-w-[110px]"
    >
...
function StatusBadge({ status }: { status: PendaftaranRow["status"] }) {
  ...
  return (
    <span
      className={`inline-block rounded-full text-xs font-bold px-2.5 py-0.5 ${cfg.className}`}
    >
      {cfg.label}
    </span>
  );
...
function ActionButtons({
  ...
  const buttons = [
    ...
    },
    {
      ...
      idleClass: "bg-yellow-50 text-yellow-600 hover:bg-yellow-100",
    },
  ];
    ...
    <div className="flex items-center gap-1">
      {buttons.map((btn) => (
        <button
          ...
          disabled={isUpdating || isDeleting || row.status === btn.status}
          className={[
            "w-7 h-7 rounded-lg text-xs font-bold transition-colors disabled:cursor-not-allowed",
            row.status === btn.status ? btn.activeClass : btn.idleClass,
            isUpdating || isDeleting ? "opacity-40" : "",
          ].join(" ")}
          title={`Set ${btn.status}`}
        >
          {btn.label}
        </button>
      ))}
            ...
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/80">
                {TABLE_HEADERS.map((h) => (
                  <th
                    key={h}
                    className="px-3 py-3 text-left text-xs font-bold text-gray-500 whitespace-nowrap"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {loading ? (
                ...
                rows.map((row, i) => (
                  ...
                  >
                    <td className="px-3 py-3 text-gray-400 font-bold text-xs">
                      {i + 1}
                    </td>
                    ...
                    <td className="px-3 py-3">
                      <span className="rounded-full bg-blue-50 text-blue-700 text-xs font-bold px-2 py-0.5 whitespace-nowrap">
                        {row.bidang1}
                      </span>
                    </td>

                    <td className="px-3 py-3">
                      <span className="rounded-full bg-purple-50 text-purple-700 text-xs font-bold px-2 py-0.5 whitespace-nowrap">
                        {row.bidang2}
                      </span>
                    </td>
